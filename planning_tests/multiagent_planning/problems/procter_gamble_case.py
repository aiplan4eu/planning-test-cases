...
class UPProcterAndGamble(TestCaseProblem):
    ...
    def get_problem(self):
        ...
        sense_imaging.add_precondition(Or( isP1(pouch),   Or(Not(detected(d4_r2_c5)), Not(pouchAt(d4_r2_c5, drawer)))   ))
        sense_imaging.add_precondition(Or( isP2(pouch),   Or(Not(detected(d4_r5_c5)), Not(pouchAt(d4_r5_c5, drawer)))   ))
        sense_imaging.add_effect(detected(pouch), True)
        sense_imaging.add_effect(pouchAt(pouch, drawer), True)


        pick_pouch = InstantaneousAction("pick_pouch", pouch=PouchObj, whereP=LocationP, gripPos=LocationG)
        ...
        put_pouch_on = InstantaneousAction("put_pouch_on", pouch=PouchObj, whereP=LocationP, gripPos=LocationG)
        ...
        drop_pouch.add_precondition(Or( And(grasped(pouch), Not(inStateG(available)), isBinLoc(gripPos)), And(measuredAt(pouch, markS1), pouchAt(pouch, markS1), isMarkSloc(gripPos)) ))  #Equals(gripPos, binLoc) #Equals(gripPos, mark10SLoc)
