...
from unified_planning.solvers.results import SOLVED_OPTIMALLY, UNSOLVABLE_PROVEN, UNSOLVABLE_INCOMPLETELY
...
class TestOptimal(TestCase):
    ...
    def _run_solver_and_check_result(self, planner, problem):
        ...
        if result.plan:
            ...
            assert result.status in (UNSOLVABLE_PROVEN,
                                     UNSOLVABLE_INCOMPLETELY)
            if result.status == UNSOLVABLE_PROVEN:
    ...
    def test_optimal_on_solvable(self):
        ...
        up_problem = problem.get_problem()
        optimal = unified_planning.solvers.solver.SOLVED_OPTIMALLY
        planner_names = [n for n, s in get_env().factory.solvers.items()
